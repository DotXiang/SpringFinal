...
public class GameTypeDatabaseDAO implements GameTypeDAO{
	...
	public GameType getGameType(String name) {
		...
		List<Map<String,Object>> rows=template.queryForList("SELECT * FROM gametype WHERE type_name=?",name);
	...
	public List<GameType> getAllGameType() {
		...
		List<Map<String,Object>> rows=template.queryForList("SELECT * FROM gametype");
		for(Map<String,Object> row:rows){
	...
	public List<Game> getAllGamesOfType(String name) {
		...
		List<Map<String,Object>> rows=template.queryForList("SELECT * FROM game WHERE game_type=?",name);
		for(Map<String, Object> row:rows){
			Game game=new Game();
	...
	public void addGameType(GameType gameType) {
		...
		if(gameType!=null){
			template.update("INSERT INTO gametype (type_name) VALUES (?)",gameType.getName());
		}
	}
